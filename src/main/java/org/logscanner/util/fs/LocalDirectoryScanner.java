...
public class LocalDirectoryScanner {
    ...
    public synchronized void setIncludes(final String[] includes) {
        if (includes == null) {
            ...
            this.includes = Stream.of(includes)
                .map(LocalDirectoryScanner::normalizePattern).toArray(String[]::new);
        }
    }
    ...
    public synchronized void setExcludes(final String[] excludes) {
        if (excludes == null) {
            ...
            this.excludes = Stream.of(excludes).map(LocalDirectoryScanner::normalizePattern)
                    .toArray(String[]::new);
        }
    }
        ...
        if (excludes != null && excludes.length > 0) {
            if (this.excludes == null || this.excludes.length == 0) {
                ...
                this.excludes = Stream.concat(Stream.of(this.excludes),
                        Stream.of(excludes).map(LocalDirectoryScanner::normalizePattern))
                        .toArray(String[]::new);
            }
        }
    }
        ...
        try {
            synchronized (this) {
                ...
                if (basedir != null && !followSymlinks
                    ...
                    notFollowedSymlinks.add(basedir.toString());
                    basedir = null;
                }
...
//                    canonBase = basedir.getCanonicalFile();
            ...
            for (final Map.Entry<TokenizedPath, String> entry : newroots.entrySet()) {
                ...
                Path myfile = basedir != null ? basedir.resolve(currentelement) : Paths.get(currentelement);
                if (Files.exists(myfile)) {
                    ...
                    try {
                        final String path = (basedir == null)
                            ? myfile.toRealPath().toString()
                            : removeLeadingPath(canonBase, myfile);
                        if (!path.equals(currentelement) || ON_VMS) {
                            myfile = currentPath.findPath(basedir, true);
                            if (myfile != null && basedir != null) {
                                currentelement = removeLeadingPath(basedir, myfile);
                ...
                if ((myfile == null || Files.notExists(myfile)) && !isCaseSensitive()) {
                    final Path f = currentPath.findPath(basedir, false);
                ...
                if (myfile != null && Files.exists(myfile)) {
                    if (!followSymlinks && currentPath.isSymlink(basedir.toFile())) {
                        accountForNotFollowedSymlink(currentPath, myfile);
                        continue;
                    }
                    if (Files.isDirectory(myfile)) {
                    ...
                    } else if (Files.isRegularFile(myfile)) {
     ...
     */
    private String removeLeadingPath(Path leading, Path path) {
    	try {
        	Path pp = path.toRealPath();
        	pp = leading.relativize(pp);
        	return pp.toString();
        } catch (final IOException ex) {
            throw new BuildException(ex);
        }
    	
//        String l = leading.normalize().toString();
...
//        return (p.startsWith(l)) ? p.substring(l.length()) : p;
    ...
    private void processSlowScan(final String[] arr) {
        for (String element : arr) {
            ...
            if (!couldHoldIncluded(path) || contentsExcluded(path)) {
                scandir(basedir.resolve(element), path, false);
            }
        }
    }
     ...
     * @see #slowScan
     */
    protected void scandir(final Path dir, final String vpath, final boolean fast) {
     ...
     * @see #slowScan
     */
    private void scandir(final Path dir, final TokenizedPath path, final boolean fast) {
        if (dir == null) {
            throw new BuildException("dir must not be null.");
        }
        final List<Path> newfiles = new ArrayList<>(); 
        try {
            Files.list(dir).forEach(f -> newfiles.add(f));
        } catch (NotDirectoryException ex) {
            throw new BuildException(dir + "%s is not a directory.");
		} catch (IOException ex) {
            if (!Files.exists(dir)) {
                throw new BuildException(dir + DOES_NOT_EXIST_POSTFIX, ex);
            } else {
                throw new BuildException("IO error scanning directory '" + dir.toString() + "'", ex);
            }
		}
    ...
    private void scandir(final Path dir, final TokenizedPath path, final boolean fast,
                         List<Path> newfiles, final Deque<String> directoryNamesFollowed) {
        ...
        if (!followSymlinks) {
            final List<Path> noLinks = new ArrayList<>();
            for (final Path filePath : newfiles) {
                if (Files.isSymbolicLink(filePath)) {
                    final String name = vpath + filePath.toString();
                    if (Files.isDirectory(filePath)) {
                        dirsExcluded.addElement(name);
                    } else if (Files.isRegularFile(filePath)) {
                        filesExcluded.addElement(name);
                    }
                    accountForNotFollowedSymlink(name, filePath);
                } else {
                    noLinks.add(filePath);
                }
            }
            newfiles = noLinks;
        } else {
        ...
        for (int i = 0; i < newfiles.size(); i++) {
            ...
            if (!isFile) {
            	try {
					Files.list(file).forEach(p -> children.add(p));
				} catch (IOException ex) {
					//FIXME: do correct exception handling 
            ...
            if (isFile || children.isEmpty()) {
                ...
                if (followSymlinks
                    ...
                    System.err.println("skipping symbolic link "
                                       ...
                                       + " links.");
                    notFollowedSymlinks.add(file.toString());
                    continue;
                }
     ...
     */
    private void accountForIncludedFile(final TokenizedPath name, final Path file) {
     ...
     */
    private void accountForIncludedDir(final TokenizedPath name, final Path file,
                                       final boolean fast) {
    ...
    private void accountForIncludedDir(final TokenizedPath name,
                                       final Path file, final boolean fast,
    ...
    private void accountForNotFollowedSymlink(final String name, final Path file) {
        accountForNotFollowedSymlink(new TokenizedPath(name), file);
    }

    private void accountForNotFollowedSymlink(final TokenizedPath name, final Path file) {
        if (!isExcluded(name) && (isIncluded(name)
                || (Files.isDirectory(file) && couldHoldIncluded(name) && !contentsExcluded(name)))) {
            notFollowedSymlinks.add(file.toString());
        }
    }

    private void processIncluded(final TokenizedPath path,
                                 final Path file, final List<String> inc, final List<String> exc,
    ...
    protected boolean isSelected(final String name, final Path file) {
        return selectors == null
                || Stream.of(selectors).allMatch(sel -> sel.isSelected(basedir.toFile(), name, file.toFile()));
    }
     ...
     * @since Ant 1.8.0
     */
    private boolean causesIllegalSymlinkLoop(final String dirName, final Path parent,
                                             final Deque<String> directoryNamesFollowed) {
    	return false;
//FIXME: not implemented causesIllegalSymlinkLoop(...)
//        try {
//            final Path dirPath;
//            if (parent == null) {
//                dirPath = Paths.get(dirName);
//            } else {
//                dirPath = parent.resolve(dirName);
//            }
